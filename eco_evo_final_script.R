...
# My phyloseq object ( IL10_clean.rar ) details:

#   otu_table()   OTU Table:         [ 9741 taxa and 75 samples ]
#   sample_data() Sample Data:       [ 75 samples by 15 sample variables ]
#   tax_table()   Taxonomy Table:    [ 9741 taxa by 7 taxonomic ranks ]


# From Burns et al. Contribution of neutral processes to the assembly of the gut microbial communities 
  # changes over host development

# Fits the neutral model from Sloan et al. 2006 to an OTU table and returns several fitting statistics. 

# Alternatively, will return predicted occurrence frequencies for each OTU based on their abundance in the 
# metacommunity when stats=FALSE. For use in R.

# spp: A community table for communities of interest with local communities/samples as rows and taxa as columns. 
  # All samples must be rarefied to the same depth.

# pool: A community table for defining source community (optional; Default=NULL).

# taxon: A table listing the taxonomic calls for each otu, with OTU ids as row names and taxonomic classifications 
  # as columns.
...
spp <- IL10_clean.rar@otu_table
taxon <- IL10_clean.rar@tax_table
pool=NULL
...
# Fit neutral model parameter m (or Nm) using Maximum likelihood estimation (MLE)
  # put calculations for d, N, and p in line 80 ^
    # d, N, and p are not in the function environment. adding them to the function environment might help
  ...
  # below are some lines of code I've tried to fix the error i'm getting in calculating MLE
    # but these aren't working :( 
   m.mle <- mle(sncm.LL, start = formals(sncm.LL), 
               optim(par, method = "Nelder-Mead", lower = -Inf, upper = Inf, hessian = FALSE), nobs = length(p))
